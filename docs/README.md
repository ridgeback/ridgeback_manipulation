...
```
<xacro:include filename="$(find ridgeback_ur_description)/urdf/ridgeback_dual_ur5_description.urdf.xacro" />
```

You will then want to export your URDF as the URDF_EXTRA for your robot.  This will "attach" it to the Ridgeback.  It's convenient to add all of these into a single shell script file that can be sourced later. Make sure to source the initial script as that will make sure to set any other necessary environment variables.

```
source $(catkin_find ridgeback_ur_description)/scripts/ridgeback_dual_ur5_envar.sh
export RIDGEBACK_URDF_EXTRAS=$(catkin_find your_description_package urdf/your_description.urdf.xacro --first-only)
```

### Arm Positioning
Following above should make your Ridgeback with an arm/arms attached to it.  They will be attached in the default locations.  If you need to change where the arm is mounted, export the necessary variables below.

```
RIDGEBACK_LEFT_UR_XYZ
RIDGEBACK_LEFT_UR_RPY
RIDGEBACK_RIGHT_UR_XYZ
RIDGEBACK_RIGHT_UR_RPY
```
Example:

```
export RIDGEBACK_LEFT_UR_XYZ="-0.3 0.0 0.2"
```
...
### Setup Package
The ridgeback_manipulation package has moveit configs setup already.  That config should work out-of-the-box for your robot, but you may get weird collisions or warnings about missing joints.  If your robot is custom, you can use that moveit config as a starting point for yours.

Go into the src folder in your workspace and run the command below:

```
rosrun ridgeback_ur_moveit_config customize_moveit.sh <new_package_name>
```
...
```
roslaunch ridgeback_ur_bringup ridgeback_ur_bringup.launch
```
...
```
roslaunch dummy_moveit_config ridgeback_ur_moveit_planning_execution.launch
```
